...
void AddNeovexDarkThemeOverrides(ColorMixer& mixer, const ColorProviderKey& key) {
  if (key.color_mode != ColorProviderKey::ColorMode::kDark) {
    return;
  }
  // Neovex Deep Dark Purple for the base surfaces
  mixer[kColorSysSurface] = {SkColorSetRGB(0x0b, 0x0b, 0x14)};
  mixer[kColorSysBase] = {SkColorSetRGB(0x0b, 0x0b, 0x14)};
  
  // Elevated surfaces (menus, dropdowns, hovered items)
  mixer[kColorSysSurface1] = {SkColorSetRGB(0x0d, 0x0d, 0x18)};
  mixer[kColorSysSurface2] = {SkColorSetRGB(0x12, 0x12, 0x1e)};
  mixer[kColorSysSurface3] = {SkColorSetRGB(0x16, 0x16, 0x24)};
  mixer[kColorSysSurface4] = {SkColorSetRGB(0x1a, 0x1a, 0x2e)};
  mixer[kColorSysSurface5] = {SkColorSetRGB(0x1d, 0x1d, 0x33)};
  
  mixer[kColorSysBaseContainer] = {SkColorSetRGB(0x12, 0x12, 0x1e)};
  mixer[kColorSysBaseContainerElevated] = {SkColorSetRGB(0x1a, 0x1a, 0x2e)};
  mixer[kColorSysNeutralContainer] = {SkColorSetRGB(0x1a, 0x1a, 0x2e)};
  
  // Header and Omnibox container
  mixer[kColorSysHeader] = {SkColorSetRGB(0x0b, 0x0b, 0x14)};
  mixer[kColorSysHeaderContainer] = {SkColorSetRGB(0x0b, 0x0b, 0x14)};
  mixer[kColorSysOmniboxContainer] = {SkColorSetRGB(0x1a, 0x1a, 0x2e)};
}
...
void AddSysColorMixer(ColorProvider* provider, const ColorProviderKey& key) {
  ...
  // === CUSTOM THEME: Enforce Neovex theme globally ===
  AddNeovexDarkThemeOverrides(mixer, key);
}
