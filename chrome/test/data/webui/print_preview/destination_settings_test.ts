...
suite('DestinationSettingsTest', function() {
  ...
  // the state.
  test('ChangeDropdownState', async () => {
    ...
    assertEquals(DestinationState.ERROR, destinationSettings.destinationState);
  ...
  test('RecentDestinationsMissing', async () => {
    recentDestinations = destinations.slice(0, 5).map(
        destination => makeRecentDestination(destination));
  ...
  // current destination.
  test('SelectRecentDestination', async () => {
    recentDestinations = destinations.slice(0, 5).map(
        destination => makeRecentDestination(destination));
    const dropdown = destinationSettings.$.destinationSelect;
  ...
  test('UpdateRecentDestinations', async function() {
    ...
    destinationSettings.getDestinationStoreForTest().startLoadAllDestinations();
    ...
    await selectDestination(destinations[1]!);
    ...
    const whenCapabilitiesDone =
        nativeLayer.waitForMultipleCapabilities(NUM_PERSISTED_DESTINATIONS);
    for (const destination of extraDestinations) {
      selectDestination(destination);
    }
    await whenCapabilitiesDone;

    assertRecentDestinations(extraDestinations.map(dest => dest.id).reverse());
    assertEquals(
