...
import type {AppManagementPermissionItemElement} from 'neovex://os-settings/lazy_load.js';
import {MediaDevicesProxy} from 'neovex://os-settings/lazy_load.js';
import type {AppManagementToggleRowElement, CrButtonElement, LocalizedLinkElement} from 'neovex://os-settings/os_settings.js';
...
suite('AppManagementPermissionItemTest', function() {
  ...
  test('Permission description updated when permission toggled', async () => {
    assertEquals(
  ...
  test('Permission description updated when no sensor connected', async () => {
    const checkPermissionDescription = async (
        permissionType: PermissionTypeIndex, expectedDescription: string) => {
