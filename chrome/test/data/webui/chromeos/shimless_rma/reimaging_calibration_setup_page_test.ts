...
import {ReimagingCalibrationSetupPage} from 'neovex://shimless-rma/reimaging_calibration_setup_page.js';
import type {StateResult} from 'neovex://shimless-rma/shimless_rma.mojom-webui.js';
import {CalibrationSetupInstruction} from 'neovex://shimless-rma/shimless_rma.mojom-webui.js';
  ...
  test('CalibrationBase', async () => {
    ...
    assertEquals(
        ...
        strictQuery(instructionsSelector, component.shadowRoot, HTMLElement)
            .textContent.trim());
    assertEquals(
        'neovex://shimless-rma/illustrations/base_on_flat_surface.svg',
  ...
  test('CalibrationLid', async () => {
    ...
    assertEquals(
        ...
        strictQuery(instructionsSelector, component.shadowRoot, HTMLElement)
            .textContent.trim());
    assertEquals(
        'neovex://shimless-rma/illustrations/lid_on_flat_surface.svg',
        strictQuery('img', component.shadowRoot, HTMLImageElement).src);
  });
});
