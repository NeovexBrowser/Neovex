...
// Handles chrome.send() messages from the custom NTP for setting the global browser theme.
class NeovexThemeMessageHandler : public content::WebUIMessageHandler {
 public:
  NeovexThemeMessageHandler() = default;
  ~NeovexThemeMessageHandler() override = default;

  void RegisterMessages() override {
    web_ui()->RegisterMessageCallback(
        "setNeovexTheme",
        base::BindRepeating(&NeovexThemeMessageHandler::HandleSetNeovexTheme,
                            base::Unretained(this)));
  }

 private:
  void HandleSetNeovexTheme(const base::ListValue& args) {
    if (args.empty() || !args[0].is_bool()) return;
    bool is_light = args[0].GetBool();
    
    ThemeService* service = ThemeServiceFactory::GetForProfile(Profile::FromWebUI(web_ui()));
    if (service) {
      if (is_light) {
        service->BuildAutogeneratedThemeFromColor(SkColorSetRGB(255, 255, 255));
      } else {
        service->UseDefaultTheme();
      }
      service->SetBrowserColorScheme(is_light ? ThemeService::BrowserColorScheme::kLight
...
class StudyModeMessageHandler : public content::WebUIMessageHandler {
 public:
  StudyModeMessageHandler() = default;
  ~StudyModeMessageHandler() override = default;

  void RegisterMessages() override {
    web_ui()->RegisterMessageCallback(
        "setStudyMode",
        base::BindRepeating(&StudyModeMessageHandler::HandleSetStudyMode,
                            base::Unretained(this)));
    web_ui()->RegisterMessageCallback(
        "setStudyModeWhitelist",
        base::BindRepeating(
            &StudyModeMessageHandler::HandleSetStudyModeWhitelist,
            base::Unretained(this)));
  }

 private:
  void HandleSetStudyMode(const base::ListValue& args) {
    if (args.size() >= 1 && args[0].is_bool()) {
      StudyModeNavigationThrottle::SetEnabled(args[0].GetBool());
    }
  }

  void HandleSetStudyModeWhitelist(const base::ListValue& args) {
    if (args.size() >= 1 && args[0].is_list()) {
      std::vector<std::string> domains;
      for (const auto& val : args[0].GetList()) {
        if (val.is_string()) {
          domains.push_back(val.GetString());
        }
      }
...
content::WebUIDataSource* CreateAndAddNewTabPageUiHtmlSource(Profile* profile) {
  ...
  source->OverrideContentSecurityPolicy(
      ...
      "'unsafe-inline';");
  // Custom NTP: Disable Trusted Types so our custom JS can do DOM manipulation
  // (e.g. createElement, appendChild, textContent) without a TrustedTypes
  // policy.
...
NewTabPageUI::NewTabPageUI(content::WebUI* web_ui)
  ...
  // Custom NTP: Theme message handler.
  web_ui->AddMessageHandler(std::make_unique<NeovexThemeMessageHandler>());
