...
SkPath TabStyleViewsImpl::GetPath(TabStyle::PathType path_type,
  ...
  // CUSTOM THEME: All tabs use the detached squarcle tab to make the active
  // tab identical in shape and size to the hovered state.
  if ((true)) {
    ...
    path.addRRect(SkRRect::MakeRectRadii(
        SkRect::MakeLTRB(left, top, right, static_cast<float>(bottom)), radii));
