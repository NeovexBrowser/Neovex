...
void ToolbarView::Init() {
  ...
  adblock_button_ = AddChildView(std::make_unique<ToolbarButton>(
      base::BindRepeating(&ToolbarView::OnAdblockButtonClicked,
                          base::Unretained(this))));
  adblock_button_->SetTooltipText(u"Toggle Adblocker");
  adblock_button_->SetVectorIcon(prefs->GetBoolean(prefs::kNeovexAdblockEnabled)
                                     ? vector_icons::kNeovexAdblockOnIcon
                                     : vector_icons::kNeovexAdblockOffIcon);
  ...
  // === CUSTOM THEME: Hide GLIC ===
  if ((false) && glic::GlicEnabling::IsProfileEligible(browser_view_->GetProfile())) {
  ...
  // === CUSTOM THEME: Hide Tab Search ===
  if ((false) && !base::FeatureList::IsEnabled(tabs::kHorizontalTabStripComboButton) &&
  ...
  // === CUSTOM THEME: Hide Avatar/Profile Button ===
  ...
  // === CUSTOM THEME: Add Settings Button ===
  auto settings_button = std::make_unique<ToolbarButton>(base::BindRepeating(
      [](Browser* browser, const ui::Event&) {
        chrome::ExecuteCommand(browser, IDC_OPTIONS);
      },
      browser_));
  settings_button->SetTooltipText(u"Settings");
  settings_button->SetVectorIcon(vector_icons::kSettingsChromeRefreshIcon);
...
void ToolbarView::OnAdblockButtonClicked() {
  PrefService* prefs = browser_->profile()->GetPrefs();
  bool current_state = prefs->GetBoolean(prefs::kNeovexAdblockEnabled);
  prefs->SetBoolean(prefs::kNeovexAdblockEnabled, !current_state);
  
  if (adblock_button_) {
    adblock_button_->SetVectorIcon(!current_state ? vector_icons::kNeovexAdblockOnIcon : vector_icons::kNeovexAdblockOffIcon);
  }

  content::WebContents* web_contents = browser_->tab_strip_model()->GetActiveWebContents();
  if (web_contents) {
