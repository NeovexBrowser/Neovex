...
#include "base/base64.h"  // NEOVEX VIRUS CHECK
...
const std::map<int, int>& GetIdcToUmaMap(UmaEnumIdLookupType type) {
  ...
  static const base::NoDestructor<std::map<int, int>> kGeneralMap(
      {// NB: UMA values for 0 and 1 are detected using
       ...
       {IDC_CONTENT_CONTEXT_CHECK_VIRUS, 162},  // NEOVEX VIRUS CHECK
       // To add new items:
  ...
  static const base::NoDestructor<std::map<int, int>> kSpecificMap(
      {{IDC_CONTENT_CONTEXT_OPENLINKNEWTAB, 0},
       ...
       {IDC_CONTENT_CONTEXT_CHECK_VIRUS, 33},  // NEOVEX VIRUS CHECK
       // To add new items:
       //   - Add one more line above this comment block, using the UMA value
       //     from the line below this comment block.
       //   - Increment the UMA value in that latter line.
...
void RenderViewContextMenu::AppendLinkItems() {
  if (!params_.link_url.is_empty()) {
    ...
    // NEOVEX VIRUS CHECK: Add virus scan option for file links
    if (IsFileDownloadLink(params_.link_url)) {
      menu_model_.AddItemWithStringId(
          IDC_CONTENT_CONTEXT_CHECK_VIRUS,
          IDS_CONTENT_CONTEXT_CHECK_VIRUS);
    }
  }
...
bool RenderViewContextMenu::IsCommandIdEnabled(int id) const {
  ...
  switch (id) {
    ...
    // NEOVEX VIRUS CHECK
    case IDC_CONTENT_CONTEXT_CHECK_VIRUS:
      return true;
...
void RenderViewContextMenu::ExecuteCommand(int id, int event_flags) {
  ...
  switch (id) {
    ...
    // NEOVEX VIRUS CHECK: Encode URL and open MetaDefender
    case IDC_CONTENT_CONTEXT_CHECK_VIRUS: {
      std::string url_str = params_.link_url.spec();
      std::string encoded = base::Base64Encode(url_str);
      GURL scan_url("https://metadefender.com/results/url/" + encoded);
      OpenURLParams open_params(
          scan_url, content::Referrer(),
          WindowOpenDisposition::NEW_FOREGROUND_TAB,
          ui::PAGE_TRANSITION_LINK,
          /*is_renderer_initiated=*/false);
      source_web_contents_->OpenURL(open_params,
                                    /*navigation_handle_callback=*/{});
      break;
    }
...
// NEOVEX VIRUS CHECK: Check if link points to downloadable file
bool RenderViewContextMenu::IsFileDownloadLink(const GURL& url) const {
  ...
  for (const char* ext : kFileExtensions) {
    if (base::EndsWith(path, ext,
