...
def WriteCab(output_file, input_file, cab_stored_filename, input_size,
    ...
    the mtime timestamp of the input file (must be at least midnight
    1980-1-1)."""
    ...
    # Many 8 bytes CFDATA blocks, representing 32kB chunks of uncompressed
    # data, each followed by the compressed data.
    cffile_offset = 36 + 8
    ...
    CFDATA = (
        '<'
        # checksum. Optional and expensive to compute in Python, so write 0.
        'I'
    ...
    for _ in range(num_chunks):
        ...
        # Have to use compressobj instead of compress() so we can pass a
        # negative window size to remove header and trailing checksum.
        # Compression level 6 runs about 8x as fast as makecab.exe's LZX
        # compression while producing a 45% larger file.  (Interestingly,
        # it also runs about 5x as fast as makecab.exe's MSZIP compression
        # while being about 4.8% larger -- so it might be possible to write
        # an LZX compressor that's much faster without being so much larger.)
        # Compression level 9 isn't very different.  Level 1 is another ~30%
